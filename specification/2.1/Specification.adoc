...
The order of joints is defined by the `skin.joints` array and it **MUST** match the order of `inverseBindMatrices` accessor elements (when the latter is present). The `skeleton` property (if present) identifies the skeleton root of the skin: the common root of the skin's joints hierarchy or an ancestor thereof. The skeleton root **SHOULD** be defined. When defined, it **SHOULD** be an ancestor of every node whose `skin` property references this skin, it **SHOULD** be the lowest common ancestor of the skin's joints and the nodes that reference the skin, and it **MAY** itself be a joint node.
...
Skinned mesh nodes **SHOULD** be placed as descendants of a common animated ancestor of the skin's joints: typically a child of the skeleton root identified by `skin.skeleton` or, when `skeleton` is not defined, a child of the _common root_. Such placement allows translations and rotations animated on ancestor nodes of the joint hierarchy to be inherited by the mesh node. Skins that declare the same `skeleton` belong to the same skeleton. Within a skeleton, skins **MAY** declare disjoint, overlapping, or identical `joints` arrays, and each skin's `JOINTS_n` attributes are indexed by that skin's own `joints` array.
